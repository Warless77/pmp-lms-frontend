...
function MockExam() {
  const [phase, setPhase] = useState('welcome'); const [questions, setQuestions] = useState([]); const [sessionId, setSessionId] = useState(null);
  const [current, setCurrent] = useState(0); const [answers, setAnswers] = useState({}); const [secondsRemaining, setSecondsRemaining] = useState(PMP_DURATION_SECONDS);
  const [error, setError] = useState(''); const [result, setResult] = useState(null); const [submitting, setSubmitting] = useState(false); const [entitlement, setEntitlement] = useState(null); const [entitlementLoading, setEntitlementLoading] = useState(true);

  useEffect(() => { getLearnerEntitlement().then(setEntitlement).catch(() => setEntitlement(null)).finally(() => setEntitlementLoading(false)); }, []);
  useEffect(() => { if (phase !== 'exam') return undefined; const timer = window.setInterval(() => setSecondsRemaining((time) => Math.max(0, time - 1)), 1000); return () => window.clearInterval(timer); }, [phase]);

  const start = async () => {
    setError(''); setSubmitting(true);
    try { const session = await startMockExam(PMP_DURATION_SECONDS); setSessionId(session.sessionId); setQuestions(session.questions); setAnswers({}); setCurrent(0); setSecondsRemaining(Math.max(0, Math.floor((new Date(session.expiresAt).getTime() - Date.now()) / 1000))); setPhase('exam'); }
    catch (e) { setError(e?.message || 'We could not prepare this mock exam.'); }
  ...
  const finish = async () => {
    ...
    try { const submission = Object.fromEntries(Object.entries(answers).map(([index, answer]) => [questions[Number(index)].id, answer])); setResult(await submitMockExamSession(sessionId, submission)); setPhase('results'); }
    catch (e) { setError(e?.message || 'We could not save this exam.'); }
    finally { setSubmitting(false); }
  };

  useEffect(() => { if (phase === 'exam' && secondsRemaining === 0 && !submitting) finish(); }, [secondsRemaining, phase, submitting]);
