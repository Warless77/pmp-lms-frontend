...
function MockExam() {
  ...
  const finish = async () => {
    ...
    if (!Object.keys(answers).length) { setError('Select at least one answer before submitting.'); return; }
    setSubmitting(true);
    try {
  ...
  const reset = () => { setPhase('welcome'); setCurrent(0); setAnswers({}); setResult(null); setSecondsRemaining(BETA_DURATION_SECONDS); };
  ...
  if (phase === 'results') return <div><PageHeader title="Mock exam complete" subtitle="Your private beta result" /><div style={{ maxWidth: '42rem', padding: '1.5rem', background: 'var(--color-surface)', border: '1px solid var(--color-border)', borderRadius: '8px' }}><h2>{result?.score || 0} / {result?.total || 0}</h2><p>You answered {Object.keys(answers).length} question{Object.keys(answers).length === 1 ? '' : 's'} in this attempt.</p><p>Your result is securely saved to your learner account.</p><button type="button" onClick={reset} style={{ padding: '0.7rem 1rem', background: 'var(--color-primary)', color: '#fff', border: 0, borderRadius: '5px', cursor: 'pointer' }}>Start a new attempt</button></div></div>;

  const question = questions[current];
  return <div><PageHeader title="Mock Exam" subtitle={`Question ${current + 1} of ${questions.length}`} />
    <div style={{ display: 'flex', justifyContent: 'space-between', gap: '1rem', alignItems: 'center', flexWrap: 'wrap', marginBottom: '1rem' }}><ExamTimer minutes={Math.floor(secondsRemaining / 60)} seconds={secondsRemaining % 60} /><span>{Object.keys(answers).length} answered</span><button type="button" disabled={submitting} onClick={finish} style={{ padding: '0.55rem 1rem', background: 'var(--color-error)', color: '#fff', border: 0, borderRadius: '5px', cursor: submitting ? 'wait' : 'pointer' }}>{submitting ? 'Submitting…' : 'Submit exam'}</button></div>
